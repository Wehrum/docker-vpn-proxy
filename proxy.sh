...
NEW_PORT="3128"  # Set the desired port
...
# Add Allow rules if they don't already exist
grep -q "Allow 127.0.0.1" "$CONFIG_FILE" || echo "Allow 127.0.0.1" >> "$CONFIG_FILE"
grep -q "Allow 10.0.0.0/8" "$CONFIG_FILE" || echo "Allow 10.0.0.0/8" >> "$CONFIG_FILE"
grep -q "Allow 172.16.0.0/12" "$CONFIG_FILE" || echo "Allow 172.16.0.0/12" >> "$CONFIG_FILE"
